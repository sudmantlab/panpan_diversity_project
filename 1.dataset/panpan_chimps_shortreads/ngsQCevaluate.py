...
SAMPLE_RUNS = np.unique([('_').join(f.split('_')[:4]) for f in READS_FILENAMES])
print(SAMPLE_RUNS)
...
bed_path = '/global/scratch/users/joana_rocha/PANPAN/reference/GCF_002880755.1_Clint_PTRv2_genomic.bed'
...
rule summarize_fastqc_raw:
    ...
    shell: 'cd evaluation/fastqc_raw && python /global/home/users/joana_rocha/scripts/summarize_fastqc.py'
...
rule summarize_fastqc_clean:
    ...
    shell: 'cd evaluation/fastqc_clean && python /global/home/users/joana_rocha/scripts/summarize_fastqc.py'

rule nbases_fastq_raw:
    input: expand('fastq_raw/{read}.fq.gz', read=READS)
    output: 'evaluation/nbases_raw.txt'
    shell: """
    for i in fastq_raw/*.fq.gz
    do
    printf $i"\t" >> {output}
