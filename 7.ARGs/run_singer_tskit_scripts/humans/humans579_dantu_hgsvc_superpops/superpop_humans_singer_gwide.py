...
POPULATIONS = "superpopulations.csv"
SAMPLES = "samples.txt"
GENE_ANNOTATION = "ht2t_CHM13_renamed_chr_gene.bed.gz"
NUM_ARGS = 99 

# 2. Robust Chunk Detection (Replaces the old glob_wildcards)
vcf_paths = glob.glob("vcf_chunks/chr4_*.vcf")
existing_chunks = []

for path in vcf_paths:
    # Extracts parts from "vcf_chunks/chr4_0_5000000.vcf"
    base = os.path.basename(path).replace(".vcf", "")
    parts = base.split("_")
    if len(parts) == 3:
        existing_chunks.append((parts[0], parts[1], parts[2]))

# Define the chromosomes to process based on what we found
chrs = sorted(list(set(c for c, s, e in existing_chunks)))

# 3. Helper functions and Dictionary loads
chrom_sizes = {}
...
rule run_singer:
    input:
        vcf = "vcf_chunks/{chr}_{start}_{end}.vcf"
    output:
