...
gene_impact_file <- "svs_vep/gene_impact_wide.tsv"
...
gprofiler_file <- "gProfiler_hsapiens_6-29-2025_9-12-33 PM.csv"
gprofiler_data <- read_csv(gprofiler_file)

pritchard_file <- "Pritchard_Table.csv"
...
human_annotation_file<-read_delim("ht2t_gene.bed.gz")
...
chimp_annotation_file<-read_delim("mPanTro3_gene.bed.gz")
...
bonobo_annotation_file<-read_delim("mPanPan1_gene.bed.gz")
...
density_plot_with_regimes <- ggplot(gene_impact_with_post_mean, aes(x = post_mean)) +
  ...
  facet_wrap(~species, ncol = 3) +
  labs(
    title = "Density of sHet by VEP for SVs",
