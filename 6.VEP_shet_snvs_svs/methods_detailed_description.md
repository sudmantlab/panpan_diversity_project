...
diploid phased call sets generated by the PANPAN pipeline (PANPAN_Singer for
SNVs; truvari-merged SV call sets ≤100 kb from `sv_detection/truvari/*`):

- **Human (HPRC year-1, n = 47):** `hprc_mapped2ht2t.sorted.filtered.vcf.gz`
  (SNVs) and `hprc.concat.vcf.gz` (SVs), with CHM13 / T2T as reference.
- **Chimpanzee (n = 24):** `lifted_pantros_mapped2mPanTro3.sorted.filtered.vcf.gz`
...
### Chimpanzee reference 

Chimpanzee SNV VCFs originally called against `mPanTro3.pri.cur.20231031.fasta`
were lifted to `mPanTro3.pri.cur.20231122.fasta` to match the curated reference
used for VEP annotation. Pairwise alignments between the two reference versions
were generated with `minimap2 -cx asm5 --cs -t 8`, sorted (`sort -k6,6V -k8,8n`),
and converted to a UCSC chain file with `paf2chain`. The VCFs were then lifted
with `bcftools +liftover` (bcftools v1.20). All input lines were retained
(0 swapped, 0 rejected) for both the biallelic SNP and all-sites VCFs.
...
VEP-annotated counts plotted in Fig. 3A:
